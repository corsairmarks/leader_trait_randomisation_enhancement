...
The code in this mod polishes some ofthe code form the base game to work better and offer options that may have been forgotten when 
...
* Handles machine traits better in code
...
* Code for adding random traits generally respects the "opposites" blocks of each trait, except that scientists are not limited to only one scientific expertise
...
* Rulers can now role "From the Ranks" and "Reformer" - the army related trait makes less narrative sense to add randomly, but politicians can definately become reformers during their tenure

## Compatibility

Most of this mod is custom code, but it does replace two small parts of the base game's code.  First, it preempts the event `leader.20` that is responsible for triggering leader level-up trait randomisation.  Notably this was one of the two places that restricted researching scientists from being able to gain traits.  Second, it replaces the effect `add_random_leader_trait` that does the actual randomisation.  The majority of code improvements are here.
...
* [Retain Leaders from Integrated Subjects]() will let you choose whener you're like to keep leaders from integrated subjects or conquered/infiltrated primitives.
